...
// script/config.sh, in script/1m_conns_benchmark.sh and in
// benchcli-rust/src/config.rs - so that a framework
...
func TestFrameworkListCoversPorts(t *testing.T) {
	...
	for _, framework := range FrameworkList {
		if FrameworkLangs[framework] == "" {
			t.Errorf("%v has no language in FrameworkLangs", framework)
		}
	}
	if len(FrameworkLangs) != len(FrameworkList) {
		t.Errorf("FrameworkLangs has %d frameworks, FrameworkList has %d", len(FrameworkLangs), len(FrameworkList))
	}
