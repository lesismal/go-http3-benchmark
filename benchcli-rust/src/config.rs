...
pub const FRAMEWORKS: &[(&str, u16, u16, bool)] =
    &[("fib", 11001, 11050, true), ("quicgo", 12001, 12050, true), ("quiche", 13001, 13050, false)];
...
fn ports(framework: &str) -> Option<(u16, u16)> {
    FRAMEWORKS.iter().find(|f| f.0 == framework).map(|f| (f.1, f.2))
}
...
pub fn benchmark_addrs(framework: &str, ip: &str) -> Result<Vec<SocketAddr>, String> {
    ...
    Ok((min..=max).map(|port| SocketAddr::new(base.ip(), port)).collect())
}
...
mod tests {
    ...
    fn addrs() {
        ...
        assert_eq!(benchmark_addrs("quiche", "::1").unwrap()[0].to_string(), "[::1]:13001");
        assert!(serves_pprof("fib") && !serves_pprof("quiche"));
        assert_eq!(benchmark_addrs("fib", "[::1]").unwrap()[0].to_string(), "[::1]:11001");
        assert_eq!(control_addr("fib", "127.0.0.1").unwrap().to_string(), "127.0.0.1:11051");
