...
const (
	...
	// then CPU EER, then MEM EER for BenchEcho and BenchMultiplex, whose TPS
	// is the responses the client read back off the server per second. The fields tagged rank:"1", rank:"2" and so on are
...
// spent, TPS / CPU Avg, or 0 when there is nothing to divide by.
func CPUEER(throughput, cpuAvg float64) float64 {
	return eer(throughput, cpuAvg)
}

// MEMEER is the throughput a server got for each MB (1024*1024 bytes) of
// resident memory it held, TPS / MEM Avg in MB, or 0 when there is nothing to
// divide by.
func MEMEER(throughput float64, memAvg uint64) float64 {
	return eer(throughput, float64(memAvg)/(1024*1024))
}

// eer is throughput / cost, or 0 when there is nothing to divide by. Every
// EER goes through this rather than dividing for itself: a server whose CPU or
// memory samples did not arrive leaves CPUAvg or MEMAvg at 0, and the +Inf
// that came out of that division took the whole row out of the report file
// with it, since encoding/json refuses to marshal Inf and NaN and ToFile's
// error was dropped.
...
func ReadReports(preffix, suffix string, create func(framework string) Report) []Report {
	...
	for _, v := range config.FrameworkList {
		...
		// The client writes no Lang: which language a server is written in
		// is config's to say, next to its ports.
		if r, ok := reportItem.(interface{ SetLang(string) }); ok {
			r.SetLang(config.FrameworkLangs[v])
		}
