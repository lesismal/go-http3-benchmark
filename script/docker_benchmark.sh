...
Environment overrides:
  ...
  DOCKER_BENCH_IMAGE       Image tag (default: go-http3-benchmark:local)
  ...
  DOCKER_BENCH_CARGO_MIRROR
                           crates.io mirror for the image's cargo fetch, as a
                           sparse index URL, e.g. sparse+https://rsproxy.cn/index/
...
image=${DOCKER_BENCH_IMAGE:-go-http3-benchmark:local}
...
if [ "$smoke" = true ]; then
    run_frameworks=quicgo
    smoke_args=(-c=100 -dc=20 -ec=50 -en=2000 -b=512 -rate=true -rd=1 -rr=20 -ep=false -rp=false)
...
mkdir -p "$result_dir"
container="go-http3-benchmark-${timestamp}-$$"
cleanup_container() {
...
run_args=(
    ...
    # Every client connection is a UDP socket with an ephemeral port of its
